...
import { ChevronLeft, ChevronRight, Moon, Sun, Monitor, MapPin, Star, Shield, MessageCircle, Heart, Eye, AlertTriangle } from "lucide-react"
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center space-x-2 mb-2">
            <span
              className={`px-3 py-1 rounded-full text-sm font-medium ${
                postData.type === "seller" ? "bg-green-100 text-green-800" : "bg-blue-100 text-blue-800"
              }`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          {postData.type === "seller" && (
            <div className="lg:col-span-3">
              <div className={`${getCardClasses()} rounded-lg overflow-hidden mb-4`}>
                <div className="relative">
                  <img
                    src={postData.item.images[selectedImageIndex] || "/placeholder.svg"}
                  ...
                  {postData.item.images.length > 1 && (
                    <>
                      ...
                      <button
                        onClick={() =>
                          setSelectedImageIndex(Math.min(postData.item.images.length - 1, selectedImageIndex + 1))
                        }
                        ...
                        disabled={selectedImageIndex === postData.item.images.length - 1}
                      >
          ...
          {/* === ITEM DETAILS === */}
          <div className={postData.type === "seller" ? "lg:col-span-2" : "lg:col-span-5"}>
            <div className={`${getCardClasses()} rounded-lg p-6 mb-6`}>
              <h3 className={`text-lg font-semibold ${getTextClasses()} mb-4`}>Item Details</h3>

              {/* 가격 */}
              <div className="mb-4">
                <span className={`text-2xl font-bold ${getTextClasses()}`}>$385</span>
              </div>
              ...
              <div className="flex items-center space-x-4 mb-4">
                ...
                <span className={`${getSecondaryTextClasses()}`}>|</span>
                <div className={`text-sm ${getSecondaryTextClasses()}`}>1 days ago</div>
                <span className={`${getSecondaryTextClasses()}`}>|</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-1">
                  <span className={`${getSecondaryTextClasses()}`}>Condition:</span>
                  <span className={`font-medium ${getTextClasses()}`}>New</span>
                </div>
                <div className="flex items-center space-x-1">
                  <span className={`${getSecondaryTextClasses()}`}>Size:</span>
                  <span className={`font-medium ${getTextClasses()}`}>US 9</span>
                </div>
                <div className="flex items-center space-x-1">
                  <span className={`${getSecondaryTextClasses()}`}>Shipping Cost:</span>
                  <span className={`font-medium ${getTextClasses()}`}>Free</span>
                </div>
                <div className="flex items-center space-x-1">
                  <span className={`${getSecondaryTextClasses()}`}>Country of Origin:</span>
                  <span className={`font-medium ${getTextClasses()}`}>United States</span>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
          ...
          <div className="lg:col-span-2">
            <div className={`${getCardClasses()} rounded-lg p-6`}>
              <h3 className={`text-lg font-semibold ${getTextClasses()} mb-4`}>Description</h3>
        ...
        <div className={`${getCardClasses()} rounded-lg p-6 mt-8`}>
          <h3 className={`text-xl font-semibold ${getTextClasses()} mb-6`}>Other Sellers</h3>
            ...
            {[
              {
                ...
                description: "Deadstock condition, stored in climate controlled environment.",
              ...
              },
              {
                ...
                price: "$320",
                thumbnail: "/placeholder.svg?height=200&width=200",
                description: "Worn 3 times, excellent condition with minimal signs of wear.",
                location: "Chicago, IL",
                hearts: 156,
              },
              {
                ...
                price: "$410",
                thumbnail: "/placeholder.svg?height=200&width=200",
                description: "New with box, authentic guarantee. Fast shipping available.",
                location: "Miami, FL",
                hearts: 189,
              },
              {
                ...
                price: "$375",
                thumbnail: "/placeholder.svg?height=200&width=200",
                description: "Pristine condition, comes with original packaging and tags.",
                location: "Seattle, WA",
                hearts: 298,
              },
            ].map((seller) => (
              <div key={seller.id} className="group cursor-pointer">
                <div className="relative mb-3">
                  ...
                  <span
                    className={`absolute top-2 left-2 text-xs px-2 py-1 rounded-full ${
                      seller.condition === "New" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                    }`}
                  >
                    {seller.condition}
                  </span>
                ...
                <div className="space-y-1">
                  <h4 className={`font-medium ${getTextClasses()} text-sm line-clamp-1`}>{seller.user}</h4>
                  <p className={`text-xs ${getSecondaryTextClasses()}`}>Size: {seller.size}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-1">
                      <Heart className="h-3 w-3 text-red-500 fill-current" />
                      <span className={`text-xs ${getSecondaryTextClasses()}`}>{seller.hearts}</span>
                    </div>
                    <span className={`text-sm font-bold ${getTextClasses()}`}>{seller.price}</span>
                  </div>
                </div>
              </div>
            ))}
