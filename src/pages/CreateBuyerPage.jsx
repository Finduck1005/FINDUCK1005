...
import { Moon, Sun, Monitor, ChevronDown, MapPin, DollarSign, AlertCircle, Eye, Save, Send } from "lucide-react"
...
export default function CreateBuyerPost() {
  ...
  // 선택 옵션 데이터
  const brands = ["Nike", "Adidas", "Jordan", "Converse", "New Balance", "Vans", "Puma", "Reebok", "Other"]
  const categories = ["Sneakers", "Clothing", "Accessories", "Collectibles"]
  const conditions = ["New with box", "New without box", "Used - Excellent", "Used - Good", "Used - Fair", "Any"]
  const sizes = [
    "US 6", "US 6.5", "US 7", "US 7.5", "US 8", "US 8.5", "US 9", "US 9.5", "US 10", "US 10.5", "US 11", "US 11.5", "US 12", "US 12.5", "US 13", "Any"
  ]
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${getThemeClasses()}`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        {/* 페이지 헤더 */}
        <div className="mb-8">
          <h1 className={`text-3xl font-bold ${getTextClasses()} mb-2`}>Create New Buying Post</h1>
          <p className={`${getSecondaryTextClasses()}`}>
            Fill out the details below to create your buying post. All fields marked with * are required.
          </p>
        </div>
        ...
        <form className="space-y-8">
          ...
          <div className={`${getCardClasses()} rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold ${getTextClasses()} mb-4`}>Item Details</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* 제목 */}
              <div className="md:col-span-2">
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Item Name *</label>
                <input
                  type="text"
              ...
              {/* 브랜드 */}
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Brand *</label>
              ...
              {/* 카테고리 */}
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Category *</label>
                <div className="relative">
                  <select
              ...
              {/* 모델 */}
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Model</label>
                <input
                  type="text"
              ...
              {/* 원하는 사이즈 */}
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Size Wanted *</label>
                <div className="relative">
                  <select
                    value={formData.sizeWanted}
                    onChange={(e) => handleInputChange("sizeWanted", e.target.value)}
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 appearance-none ${
                      theme === "dark" ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300"
                    }`}
                  >
              ...
              {/* 선호하는 상태 */}
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Condition Preference *</label>
                <div className="relative">
                  <select
                    value={formData.conditionPreference}
                    onChange={(e) => handleInputChange("conditionPreference", e.target.value)}
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 appearance-none ${
                      theme === "dark" ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300"
                    }`}
                  >
          ...
          {/* 예산 */}
          <div className={`${getCardClasses()} rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold ${getTextClasses()} mb-4 flex items-center`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Max Budget *</label>
                <div className="relative">
          ...
          {/* 설명 */}
          <div className={`${getCardClasses()} rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold ${getTextClasses()} mb-4`}>Description *</h2>
            <textarea
            ...
            />
            <p className={`text-sm ${getSecondaryTextClasses()} mt-2`}>{formData.description.length}/1000 characters</p>
          </div>
          ...
          <div className={`${getCardClasses()} rounded-lg p-6`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className={`block text-sm font-medium ${getTextClasses()} mb-2`}>Preferred Location</label>
                <input
                  type="text"
              ...
              <div className="flex items-center mt-2 md:mt-0">
                <input
                  ...
                  onChange={(e) => handleInputChange("meetupAvailable", e.target.checked)}
                  className={`h-5 w-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500 ${
                    theme === "dark" ? "bg-gray-700 border-gray-600" : ""
                  }`}
                />
                <label htmlFor="meetupAvailable" className={`ml-2 block text-sm ${getTextClasses()}`}>
                  Meetup Available
                </label>
              </div>
            </div>
          ...
          <div className={`${getCardClasses()} rounded-lg p-6`}>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              <p className={`${getSecondaryTextClasses()}`}>
                Your buying post preview will appear here once you fill out the required fields
              </p>
            </div>
          </div>
          ...
          {/* 팁 */}
          <div className={`${getCardClasses()} rounded-lg p-6 border-l-4 border-blue-500`}>
            <div className="flex items-start space-x-3">
              <AlertCircle className="h-5 w-5 text-blue-500 mt-0.5" />
              <div>
                <h3 className={`font-semibold ${getTextClasses()} mb-2`}>Tips for a successful buying post:</h3>
                <ul className={`text-sm ${getSecondaryTextClasses()} space-y-1`}>
                  <li>• Be specific about the item you are looking for (model, year, color, etc.)</li>
                  ...
                  <li>• Respond to messages quickly to facilitate a smooth transaction</li>
                </ul>
              </div>
            </div>
          </div>
